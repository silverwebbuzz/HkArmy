$(function() {
        ...
        if (postTypeID != '') {
           if(postTypeID == 1){
               if($("#eventMoney").html() == undefined){
                $("#EditaddPostType").append('<div id="eventMoney" class="form-group col-md-6 mb-50 col-lg-3"><label class="text-bold-600" for="event_money">Money</label><input type="text" class="form-control" id="event_money" name="event_money" placeholder="" value=""><button type="button" class="btn btn-primary btn-sm deletepost" data-id="1">X</button></div>');
               }
           }else if(postTypeID == 2){
               if($("#eventToken").html() == undefined){
                   $("#EditaddPostType").append('<div id="eventToken" class="form-group col-md-6 mb-50 col-lg-3"><label class="text-bold-600" for="event_money">Token</label><input type="text" class="form-control" id="event_token" name="event_token" placeholder="" value=""><button type="button" class="btn btn-primary btn-sm deletepost" data-id="1">X</button></div>');
                }
           }else if(postTypeID == 3){
               $("#eventMoneyToken").show();
           }
    ...
    //$('.addMember').on('click', function(e) {
            ...
            $.ajax({
                ...
                success: function (response) {
                   /* var div = $('.assignMemberList');
                    ...
                    select.empty();
                    select.append('<option value="">'+Select_MEMBER_LAN+'</option>');
                    test = object.member;
                    $.each(test,function(key, value) {
                        select.append('<option value=' + value.ID + '>' + value.English_name + '</option>');
                    });
        ...
        $(document).on('click', '.assign_member_event', function() {
            ...
            var memberID = $("[name='membermodal1']").children("option:selected").val();
            if (memberID != '') {
                $.ajax({
                    type: 'POST',
...
$(document).on('change', '.eventTypeForCode', function() {
    ...
    **/
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $.ajax({
                        ...
                        data: {
                            '_token': $('meta[name="csrf-token"]').attr('content'),
                            filter_date_attendance_event: date,
                            type : type
                        },
                        success: function(response) {
                            $("#cover-spin").hide();
    ...
    if ($(".filter_date_enroll").length) {
        ...
        $('.filter_date_enroll').on('apply.daterangepicker', function(ev, picker) {
            $(this).val(picker.startDate.format('MM/DD/YYYY'));
            ...
            $.ajax({
                ...
                success: function(data) {
                 ...
                 $.each(data,function(key, value) {
                  select.append('<option value=' + value.id + '>' + value.event_name + '</option>');
              });
             }
         });
        });
    ...
    $(document).on("click", "#recordAttend", function() {
     ...
     $(document).on("change", "#event_id", function() {
        ...
        if (event_id != '') {
            $.ajax({
                type: "GET",
                url: BASE_URL + "/getEventAttenderList/" + event_id + '/' + date + '/' + type,
                data: {},
                success: function(response) {
                    $("#cover-spin").hide();
                /*var data = JSON.parse(JSON.stringify(response));
       ...
       if(event_type_val != ''){
         $.ajax({
            ...
            data: {
                ...
                filter_date_attendance_event: $('#filter_date_enroll').val(),
            },
            success: function(data) {
               var select = $('form select[name= filter_event_id]');
               select.empty();
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (object.status) {
                            ...
                            $('input[name="old_event_dates"]').val(object.old_dates);
    ...
    init: function() {
        ...
        if ($("#member-table").length) {
            ...
            $(document).on('click', '.assign-user-cls', function() {
                ...
                var postType = $("#events_post_type").val();
                ...
                if (useridarr != '') {
                    ...
                    if(type == 'assign_product'){
                     var costType = $("#product_cost_type").val();   
                        if (productid != '') {
                            $("#cover-spin").show();
                            $.ajax({
         ...
         $(".dt-buttons").append("<div class='event_product_assign_dropdown'><select class='form-control event_product_assign_select' name='event_product_assign_select'><option value='assign_event'>"+EVENT_LAN+"</option><option value='assign_product'>"+PRODUCT_LAN+"</option></select></div>");
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (data.status) {
                    ...
                    $.ajax({
                        type: "GET",
                        url: BASE_URL + "/getEventAttenderList/" + $('.currentevent').val() + '/' + date,
                        data: {},
                        success: function(response) {
                           $(".event-search-list-cls").html(response);
                           $("#search-eventtable").dataTable();
    ...
    $.ajax({
        ...
        success: function(doc) {
            if (doc != null) {
                ...
                var createEventID = $('input[name="update_event_id"]').val();
                ...
                $.each(doc, function(index, value) {
                 ...
                 $.each(eventScheduleData, function( key, value1 ) {
                    ...
                    if (eventnumber == value.event_code || createEventID == value.id) {
                        if(value.status == '1'){
                            var dt = new Date();
                            ...
                            const convertedDate = convertTZ(dt, "Asia/Hong_Kong") 
                            var time = convertedDate.getHours()+ ":" + convertedDate.getMinutes();
