					...
					<div class="col-12">
						<div class="card">
							<div class="card-content">
								<form action="{{ url('eventManagement',$edit_event['id']) }}" method="POST" id="eventform_edit" name="eventform" class="eventform" autocomplete="off">
									{{ method_field('PUT') }}
									<input type="hidden" name="_token"  id="csrf-token" value="{{ csrf_token() }}">
									<input type="hidden" name="event_main_id"   value="{{ $edit_event['id'] }}">
									...
									<div class="card-body">
										...
										<div class="form-row">
											...
											<div class="form-group col-md-3 mb-50">
												<div class="form-row">
													<div class="form-group col-md-12 mb-50">
														<label class="text-bold-600">No Of. Dates</label>
														<input type="text" class="form-control" placeholder="No Of Dates" value="{{ $edit_event['no_of_dates'] }}" name="no_of_dates" readonly>
													</div>
												</div>
											</div>
										</div>
										...
										<div class="form-row">
											<div class="form-group col-md-6 mb-50">
												...
												<fieldset class="form-group">
													<select class="form-control editeventPostType" id="postType" name="post_type">
														<option value="">{{ __('languages.event.Select_post_type') }} </option>
														<option value="" <?php if ($edit_event['is_free_event'] == '1' || $postType->isEmpty() ) {echo 'selected';}?>> {{ __('languages.free') }}</option>
											...
											<input type="hidden" name="old_event_dates" value="{{ $edit_event['schedule_dates'] }}">
											<div class="form-group col-md-3 mb-50 event_date_main_cls" style="<?php if ($edit_event['status'] == '1') {echo 'display: none';}?>">
											<?php
											$explodeScheduleDate = '';
											if(isset($edit_event['schedule_dates']) && !empty($edit_event['schedule_dates'])){
												$scheduledatesArr = explode(',',$edit_event['schedule_dates']);
												$newScheduleDate = [];
												foreach($scheduledatesArr as $scheduleDate){
													//$newScheduleDate[] = date("d-m-Y", strtotime(str_replace("/", "-", $scheduleDate)));
													$newScheduleDate[] = date("d/m/Y", strtotime(str_replace("/", "-", $scheduleDate)));
												}
												$explodeScheduleDate = implode(",",$newScheduleDate);
											}
											
											?>
												<label class="text-bold-600" for="event_token">{{ __('languages.event.Select Date') }}</label>
												<div class="input-group date form-group" id="event_edit_dates_cal">
													<input type="text" class="form-control" id="Dates" name="event_dates" value="{{$explodeScheduleDate}}" placeholder="{{ __('languages.event.Select Date') }}" />
													<span class="input-group-addon"><i class="bx bx-calendar"></i><span class="count"></span></span>
												</div>
												<div class="date-error-cls"></div>
											</div>
											<div class="form-group col-md-3 mb-50 event_start_end_dropdown" style="<?php if ($edit_event['status'] == '1') {echo 'display: none';}?>">
												<div class="start_time_dropdown">
													<label class="text-bold-600" for="event_token">{{ __('languages.event.Select_time') }}</label>
													<input type="text" class="form-control" placeholder="{{ __('languages.event.Select_time') }}" id="eventstarttime" name="eventstarttime" value="{{$edit_event['start_time']}}">
												</div>
												<div class="end_time_dropdown">
													<label class="text-bold-600" for="event_token"></label>
													<input type="text" class="form-control" placeholder="{{ __('languages.event.Select_time') }}" id="eventendtime" name="endtime" value="{{$edit_event['end_time']}}">
										...
										<div class="form-row">
											...
											<div id="eventMoney" class="form-group col-md-3 mb-50 col-lg-3">
												...
												@if($post->post_type == 1)
													...
													<div class="main-event-drop">
														<input type="text" class="form-control" id="event_money" name="event_money[]" placeholder="{{ __('languages.event.event_money') }}" value="{{ $post->post_value }}">
														<a class="btn btn-primary btn-sm removeMoney deletePostType" data-eventid="{{$post->event_id}}" data-postid="{{$post->id}}" data-posttype="Money">X</a>
													</div>
												@php $money++ @endphp
												@endif
												@endforeach
												</div>
												<button type="button" class="btn btn-primary btn-sm addMore" data-id="1">{{ __('languages.event.add_event_money') }}</button>
											</div>
											...
											<div id="eventToken" class="form-group col-md-3 mb-50 col-lg-3">
												...
												@foreach($postType as $post)
													...
													{{-- <label class="text-bold-600" for="event_token">{{ __('languages.event.event_token') }}</label> --}}
													<div class="main-event-drop">
														<input type="text" class="form-control" id="event_token" name="event_token[]" placeholder="{{ __('languages.event.event_token') }}" value="{{ $post->post_value }}"> 
														<a class="btn btn-primary btn-sm removeToken deletePostType" data-eventid="{{$post->event_id}}" data-postid="{{$post->id}}" data-posttype="Token">X</a>
													</div>
													@php $token++ @endphp
													@endif
												@endforeach
											...
											<div class="form-group col-md-6 mb-50 col-lg-6 moneyandtoken">
												<label class="text-bold-600" for="event_money_token">{{ __('languages.event.Event_Money_Tokens') }}</label><!--Line no 186 to line No 178 Set-->
												<div class="event-token-money-section">
													@php $i = 0 @endphp
													@foreach($postType as $post)
													@if($post->post_type == 3)
													<?php $value = explode("+", $post->post_value);?>
													<div class="add-evens-cls1 main-money-token">
														<div class="form-group col-md-5 mb-50">	
															{{-- <label class="text-bold-600" for="event_money_token">{{ __('languages.event.Event_Money_Tokens') }}</label> --}}
															<input type="text" class="form-control" id="event_plus_money" name="event_money_token[{{$i}}][money]" placeholder="{{ __('languages.event.event_money') }}" value="{{ $value[0] ?? $value[0] }}">
														</div>
														<div class="form-group col-md-5 mb-50">
															{{-- <label class="text-bold-600" for="event_money_token"></label> --}}
															<input type="text" class="form-control" id="event_plus_token" name="event_money_token[{{$i}}][token]" placeholder="{{ __('languages.event.event_token') }}" value="{{ $value[1] ?? $value[1] }}">
														</div>
														<div class="form-group col-md-2 money-token-btn">
															<!-- <a class="removeMoneyToken deletePostType btn btn-primary btn-sm">X</a> -->
															<a class="btn btn-primary btn-sm removeMoneyToken deletePostType" data-eventid="{{$post->event_id}}" data-postid="{{$post->id}}" data-posttype="Money_Token">X</a>
														</div>
													</div>
													@php $i++ @endphp
													@endif
												...
												<button type="button" class="btn btn-primary addMore MoneyTokenAdd-btn ml-1" data-id="3">{{ __('languages.event.add_event_money_token') }}</button>
											</div>
										</div>
											...
											<div class="form-row">
												...
												<!-- <div class="form-group col-md-3 mb-50">
													<label class="text-bold-600" for="event_money">{{ __('languages.event.event_token') }}</label>
													<input type="text" class="form-control" id="event_token" name="event_token" placeholder="{{ __('languages.event.event_token') }}" value="{{ $edit_event['event_token'] }}">
												</div> -->
											<!-- <div class="form-group col-md-3 mb-50"> -->
												@if(in_array('event_management_create', Helper::module_permission(Session::get('user')['role_id'])))
												<div class="form-row add-evens-cls1">
													<div class="form-group col-md-12 mb-50">
														<button type="button" class="btn btn-primary glow position-relative edit_new_event_dates" style="float: right;<?php if ($edit_event['status'] == '1') {echo 'display: none';}?>">
														{{__('languages.event.Add_dates')}}
													</button>
													</div>
												</div>
												@endif
									...
									<div class="form-row add-event-cls-bottom">
										<div class="form-group col-md-12 mb-50">
											<button type="submit" class="btn btn-primary glow position-relative">{{__('languages.Update')}}</button>
										</div>
									</div>
									@endif
								</form>
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						...
						<div class="form-row">
							<div class="form-group col-md-12 mb-50">
								<label for="users-list-role">{{ __('languages.Status') }}</label>
								<select class="form-control" id="status" name="status"> <!-- On change -->
									<option value="">{{ __('languages.event.Select_status') }}</option>
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						...
						<div class="form-row">
							...
							<label id="error-msg-editstarttimeendtime" style="display:none;">{{__('languages.please_select_valid_start_time_and_end_time')}}</label>
						</div>
						<div class="form-row">
							<div class="form-group col-md-12 mb-50">
								<label class="text-bold-600" for="TotalHours">{{ __('languages.event.Total_hour') }}</label>
...
<script>
$("#edit_event_status").change(function() {
	document.forms["eventform_edit"].submit();
});
</script>
<!-- /footer content -->
@endsection