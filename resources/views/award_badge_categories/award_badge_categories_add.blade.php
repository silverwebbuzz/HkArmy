			...
			<section class="users-edit">
				<div class="card">
					<div class="card-content">
						<form action="{{ route('awards-badges-categories.store') }}" method="POST" id="AwardBadgeCategoriesForm" name="AwardBadgeCategoriesForm" enctype='multipart/form-data'>
						<input type="hidden" name="_token" id="csrf-token" value="{{ csrf_token() }}">
							<div class="card-body">
								<div class="form-row">
									<div class="form-group col-md-6 mb-50">
										...
										<select class="form-control" id="categories_type" name="categories_type">
											...
											<option value="award">{{ __('languages.awards_badges_categories.award') }}</option>
											<option value="badge">{{ __('languages.awards_badges_categories.badge') }}</option>
										</select>
										@if($errors->has('categories_type'))
    										<span class="validation_error">{{ $errors->first('categories_type') }}</span>
										@endif
									</div>
									<div class="form-group col-md-6 mb-50" id="parent-categories-section">
										...
										<select class="form-control" id="parent_categories_id" name="parent_categories_id">
											<option value="">{{ __('languages.awards_badges_categories.select_parent_categories') }}</option>
										</select>
								...
								<div class="form-row">
									<div class="form-group col-md-6 mb-50" id="is_current_team_member_option" style="display:none;">
										...
										<select class="form-control" id="is_team_member_mentor" name="is_team_member_mentor">
											<option value="" selected>{{ __('languages.awards_badges_categories.select_categories') }}</option>
											<option value="yes">{{ __('languages.awards_badges_categories.yes') }}</option>
											<option value="no">{{ __('languages.awards_badges_categories.no') }}</option>
										</select>
								...
								<div class="form-row">
									<div class="form-group col-md-6 mb-50">
										...
										<input type="text" class="form-control" id="name_en" name="name_en" placeholder="{{ __('languages.awards_badges_categories.categories_name_en') }}" value="{{old('name_en')}}">
									...
									<div class="form-group col-md-6 mb-50">
										...
										<input type="text" class="form-control" id="award_name_ch" name="name_ch" placeholder="{{ __('languages.awards_badges_categories.categories_name_ch') }}" value="{{old('name_ch')}}">
